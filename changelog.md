...
See [new example that demonstrates concurrency](https://github.com/vadimkozak/centrifuge/v1/tree/master/_examples/concurrency) using bounded semaphore.
...
See also updated [Tips and tricks](https://github.com/vadimkozak/centrifuge/v1#tips-and-tricks) section in a README – it now contains information about connection life cycle and event handler concurrency. 
...
* `HistoryManager` interface removed and its methods now part of `Broker` interface{}. The reason behind this is that Broker should be responsible for an atomicity of saving message into history stream and publish to PUB/SUB. More details in [#158](https://github.com/vadimkozak/centrifuge/v1/pull/158)
...
* Fix reconnects due to `InsufficientState` errors when using legacy seq, gen fields - [#157](https://github.com/vadimkozak/centrifuge/v1/pull/157)
* Fix returning custom disconnect for SockJS transport
* Possibility to define history stream options in `Publish` call
* Deprecate Broker/Engine `Channels` method – see [#147](https://github.com/vadimkozak/centrifuge/v1/issues/147)
* Increase test coverage up to 83% so [#106](https://github.com/vadimkozak/centrifuge/v1/issues/106) is finally closed
* Test Sentinel scenario in CI
* Refactor queue writer to prevent possible message loss on connection close - [160](https://github.com/vadimkozak/centrifuge/v1/pull/160)
* Fix inconsistent tests of Redis Cluster recovery due to PUB/SUB buffering
* Minor improvements in Gin auth example - [#154](https://github.com/vadimkozak/centrifuge/v1/pull/154)

I have a plan for future library versions to remove `ChannelOptionFunc` completely (but still have a control over channel feature set). This is still in research – if you are interested welcome to [#161](https://github.com/vadimkozak/centrifuge/v1/issues/161).
...
* Refactor client channels API – see detailed changes below, [#146](https://github.com/vadimkozak/centrifuge/v1/pull/146)
* Fix atomic alignment in struct for 32-bit builds, [commit](https://github.com/vadimkozak/centrifuge/v1/commit/cafa94fbf4173ae46d1f5329a33adec97d0620c8)
* Field `Code` of `Disconnect` has `uint32` type now instead of `int`, [commit](https://github.com/vadimkozak/centrifuge/v1/commit/d86cea2c8b309e6d2ce1f1fa8ba6fcc7d06f7320)
* Refactor WebSocket graceful close – do not use a new goroutine for every read, [#144](https://github.com/vadimkozak/centrifuge/v1/pull/144)
* Support client name and version fields of `Connect` command which will be available in `ConnectEvent` struct (if set on client side), [#145](https://github.com/vadimkozak/centrifuge/v1/pull/145)
...
* Fix Redis Engine errors when epoch is missing inside stream/list meta hash. See [commit](https://github.com/vadimkozak/centrifuge/v1/commit/f5375855e3df8e1679e11775455597cc91b8e0e5)
...
Most work done inside [#129](https://github.com/vadimkozak/centrifuge/v1/pull/129) pr and relates to [#128](https://github.com/vadimkozak/centrifuge/v1/issues/128) issue.
...
* Centrifuge still needs to know some core options for channels to understand whether to use presence inside channels, keep Publication history stream or not. It's now done over user-defined callback function in Node Config called `ChannelOptionsFunc`. See its detailed description in [library docs](https://godoc.org/github.com/vadimkozak/centrifuge/v1#ChannelOptionsFunc).
* More idiomatic error handling in event handlers, see [#134](https://github.com/vadimkozak/centrifuge/v1/pull/134).
* Aliases to `Raw`, `Publication` and `ClientInfo` Protobuf types removed from library public API, see [#136](https://github.com/vadimkozak/centrifuge/v1/issues/136)
* Support Redis Sentinel password option

Look at updated [example in README](https://github.com/vadimkozak/centrifuge/v1#quick-example) and [examples](https://github.com/vadimkozak/centrifuge/v1/tree/master/_examples) folder to find out more. 
...
* It's now possible to do client-side refresh with custom token implementation ([example](https://github.com/vadimkozak/centrifuge/v1/tree/master/_examples/custom_token))
...
* Fix closing connections with `insufficient state` after publish when history recovery feature is on and `PublishOnHistoryAdd` is `false` in Redis Engine config. [#119](https://github.com/vadimkozak/centrifuge/v1/issues/119).
...
* use [alternative library](https://github.com/cristalhq/jwt) for JWT parsing and verification - HMAC-based JWT parsing is about 2 times faster now. Related issues: [#109](https://github.com/vadimkozak/centrifuge/v1/pull/109) and [#107](https://github.com/vadimkozak/centrifuge/v1/pull/107).
...
* fix `channels` counter update frequency ([commit](https://github.com/vadimkozak/centrifuge/v1/commit/23a87fd538e44894f220146ced47a7e946bcf60d))
...
This release introduces server-side subscription support - see [#89](https://github.com/vadimkozak/centrifuge/v1/pull/89) for details. Release highlights:
...
* Introduce subscription dissolver - node now reliably unsubscribes from PUB/SUB channels, see details in [#77](https://github.com/vadimkozak/centrifuge/v1/issues/77)
